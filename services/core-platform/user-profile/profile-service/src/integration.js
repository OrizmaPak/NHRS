...
async function checkPermission(fetchFn, { rbacBaseUrl, authorization, permissionKey, organizationId }) {
  const result = await callJson(fetchFn, `${rbacBaseUrl}/rbac/check`, {
    method: 'POST',
    headers: { authorization, 'content-type': 'application/json' },
    body: JSON.stringify({ permissionKey, organizationId: organizationId || null }),
  });
