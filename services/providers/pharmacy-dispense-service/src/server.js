...
function createApp(options = {}) {
  ...
  function emitAuditEvent(event, req) {
    ...
    return state.repository.enqueueOutboxEvent(buildEventEnvelope({
      ...
      aggregateType: event.resource?.type || 'pharmacy_dispense',
      aggregateId: event.resource?.id || event.metadata?.dispenseId || null,
      payload: event,
      trace: {
  ...
  fastify.addHook('onRequest', createContextVerificationHook({
    ...
    requiredMatcher: (req) => req.url.startsWith('/pharmacy'),
  }));
